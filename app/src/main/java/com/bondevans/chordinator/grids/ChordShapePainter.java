...
public class ChordShapePainter {
	...
	private final static int STRING_WIDTH = 1;	// Must be ODD number!!
	private static int FRETS = 5;
	private final static int FRET_HEIGHT = 1;
	private ShapeProvider mShapes;
	/**
	 * Constructor takes width in points of grid
	 */
	public ChordShapePainter(int maxWidth, Paint paint, String extraShapes, int instrument){
		...
		case ChordShapeProvider.INSTRUMENT_GUITAR:
			mShapes = new GuitarChordShapeProvider(extraShapes);
			break;
		case ChordShapeProvider.INSTRUMENT_MANDOLIN:
			mShapes = new MandolinChordShapeProvider(extraShapes);
			break;
		case ChordShapeProvider.INSTRUMENT_UKELELE:
			mShapes = new UkeleleChordShapeProvider(extraShapes);
			break;
		case ChordShapeProvider.INSTRUMENT_BANJO:
				mShapes = new TenorBanjoChordShapeProvider(extraShapes);
				break;
		}
		FRETS = mShapes.FRETS();
		initGrid(maxWidth);
	 ...
	 * for all chords
	 * @return Returns height of complete shape
	 */
